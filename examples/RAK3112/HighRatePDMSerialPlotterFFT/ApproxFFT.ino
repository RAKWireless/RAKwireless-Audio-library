...
//---> here onward out_r contains amplitude and our_in conntains frequency (Hz)
  int fout, fm;
  float fstep;
  // Calculate the bin width using the actual number of points a used for the FFT
  fstep = Frequency / (float)a;

  fout = 0; fm = 0;

  unsigned int halfA = a / 2;
  for (unsigned int i = 1; i < halfA; i++)      // getting amplitude from complex number, only up to Nyquist
  {
  ...
  // To prevent out-of-bounds, perform 3 more interpolation points to find a more accurate peak position
  if (fm < 1) fm = 1;
  if (fm > (int)(halfA - 2)) fm = halfA - 2;

  float fa = out_r[fm - 1];
  float fb = out_r[fm];
  float fc = out_r[fm + 1];

  // Interpolate to find the exact location of the bin (fractional bin)
  float interpolatedBin = (fa * (fm - 1) + fb * fm + fc * (fm + 1)) / (fa + fb + fc);

  return (interpolatedBin * fstep);
}
