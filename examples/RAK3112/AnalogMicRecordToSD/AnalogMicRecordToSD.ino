/**
   ...
   First, you need to prepare the hardware module and SD card.
    Step 1: Compile and download the example to RAK3112.
    Part 2: Reset RAK3112 and open the serial port.
...
void setup() {
  ...
  if (!I2S.begin(I2S_MODE_STD, frequency, (i2s_data_bit_width_t)BIT_PER_SAMPLE, (i2s_slot_mode_t)channels)) 
  {
...
void microphone_record(const char* song_name, uint32_t duration) 
{
    ...
    uint32_t data_bytes = frequency * BIT_PER_SAMPLE / 8 * duration; 
    ...
    while (counter < data_bytes) 
    {
        int availableBytes = I2S.available();
        if (availableBytes == 0) continue;

        int bytesToRead = (availableBytes > fifo_size) ? fifo_size : availableBytes;
        int bytesRead = I2S.readBytes((char*)buf, bytesToRead);
        ...
        int bytesToWrite = monoSamples * 2; 
        audio_file.write(buf, bytesToWrite);
